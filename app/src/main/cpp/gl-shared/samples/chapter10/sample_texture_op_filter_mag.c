...
} Extension_TextureOpMagFilter;

/**
 * アプリの初期化を行う
 */
void sample_TextureOpMagFilter_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_TextureOpMagFilter*) malloc(sizeof(Extension_TextureOpMagFilter));
    // サンプルアプリ用データを取り出す
    Extension_TextureOpMagFilter *extension = (Extension_TextureOpMagFilter*) app->extension;
...

    {
        ...
        // 画像ピクセルを読み込む
        {
            ...
            image = RawPixelImage_load(app, "texture_rgb_32x32.png", TEXTURE_RAW_RGBA8);
            // 正常に読み込まれたかをチェック
            assert(image != NULL);
 ...
 * レンダリングエリアが変更された
 */
void sample_TextureOpMagFilter_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_TextureOpMagFilter_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_TextureOpMagFilter *extension = (Extension_TextureOpMagFilter*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_TextureOpMagFilter_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_TextureOpMagFilter *extension = (Extension_TextureOpMagFilter*) app->extension;
