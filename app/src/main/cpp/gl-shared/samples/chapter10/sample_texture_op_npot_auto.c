...
} Extension_TextureOpNpotAuto;

/**
 * アプリの初期化を行う
 */
void sample_TextureOpNpotAuto_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_TextureOpNpotAuto*) malloc(sizeof(Extension_TextureOpNpotAuto));
    // サンプルアプリ用データを取り出す
    Extension_TextureOpNpotAuto *extension = (Extension_TextureOpNpotAuto*) app->extension;
...

    {
        ...
        // 拡張機能有無によってラッピング方法を変更する
        {
            ...
            __logf("Extensions :: %s", EXTENSIONS);

            if (strstr(EXTENSIONS, "GL_OES_texture_npot")) {
                __log("Support NPOT");
 ...
 * レンダリングエリアが変更された
 */
void sample_TextureOpNpotAuto_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_TextureOpNpotAuto_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_TextureOpNpotAuto *extension = (Extension_TextureOpNpotAuto*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_TextureOpNpotAuto_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_TextureOpNpotAuto *extension = (Extension_TextureOpNpotAuto*) app->extension;
