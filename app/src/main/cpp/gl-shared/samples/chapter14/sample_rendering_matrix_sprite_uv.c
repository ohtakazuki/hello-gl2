...
typedef struct {
    ...
    // 描画対象のテクスチャ
    Texture *texture;
} Extension_ShaderMatrixSpriteUV;

/**
 * アプリの初期化を行う
 */
void sample_ShaderMatrixSpriteUV_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_ShaderMatrixSpriteUV*) malloc(sizeof(Extension_ShaderMatrixSpriteUV));
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSpriteUV *extension = (Extension_ShaderMatrixSpriteUV*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixSpriteUV_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixSpriteUV_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSpriteUV *extension = (Extension_ShaderMatrixSpriteUV*) app->extension;
    ...
    // 四角形描画
    {
        ...
        // UV行列をアップロードする
        {
            ...
            // スケーリングを行う
            const GLfloat xScale = (GLfloat) image.width / (GLfloat) extension->texture->width;
            const GLfloat yScale = (GLfloat) image.height / (GLfloat) extension->texture->height;

            const mat4 scale = mat4_scale(xScale, yScale, 0);

            // 移動を行う
            const GLfloat xMove = (GLfloat) image.x / (GLfloat) extension->texture->width;
            const GLfloat yMove = (GLfloat) image.y / (GLfloat) extension->texture->height;
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixSpriteUV_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSpriteUV *extension = (Extension_ShaderMatrixSpriteUV*) app->extension;
