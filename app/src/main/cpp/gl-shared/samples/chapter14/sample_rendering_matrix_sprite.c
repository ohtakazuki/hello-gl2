...
typedef struct {
    ...
    // 描画対象のテクスチャ
    Texture *texture;
} Extension_ShaderMatrixSprite;

/**
 * アプリの初期化を行う
 */
void sample_ShaderMatrixSprite_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_ShaderMatrixSprite*) malloc(sizeof(Extension_ShaderMatrixSprite));
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSprite *extension = (Extension_ShaderMatrixSprite*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixSprite_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixSprite_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSprite *extension = (Extension_ShaderMatrixSprite*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixSprite_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSprite *extension = (Extension_ShaderMatrixSprite*) app->extension;
