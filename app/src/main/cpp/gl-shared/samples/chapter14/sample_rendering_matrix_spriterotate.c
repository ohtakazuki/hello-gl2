...
typedef struct {
    ...
    // 描画対象のテクスチャ
    Texture *texture;
} Extension_ShaderMatrixSpriteRotate;

/**
 * アプリの初期化を行う
 */
void sample_ShaderMatrixSpriteRotate_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_ShaderMatrixSpriteRotate*) malloc(sizeof(Extension_ShaderMatrixSpriteRotate));
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSpriteRotate *extension = (Extension_ShaderMatrixSpriteRotate*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixSpriteRotate_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixSpriteRotate_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSpriteRotate *extension = (Extension_ShaderMatrixSpriteRotate*) app->extension;
    ...
    // 四角形描画
    {
        ...
        // 行列をアップロードする
        {
            ...
            const GLint spriteY = 256;
            const GLint spriteWidth = 512;
            const GLint spriteHeight = 256;
            ...
            const GLfloat yScale = (GLfloat) spriteHeight / (GLfloat) app->surface_height * 2.0f;
            ...
            // 描画用の行列を生成する translate <- rotate <- scale順
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixSpriteRotate_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSpriteRotate *extension = (Extension_ShaderMatrixSpriteRotate*) app->extension;
