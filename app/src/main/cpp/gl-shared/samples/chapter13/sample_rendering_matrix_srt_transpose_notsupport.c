...
typedef struct {
    ...
    // 描画対象のテクスチャ
    Texture *texture;
} Extension_ShaderMatrixSRTTransposeNotsupport;

/**
 * アプリの初期化を行う
 */
void sample_ShaderMatrixSRTTransposeNotsupport_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_ShaderMatrixSRTTransposeNotsupport*) malloc(sizeof(Extension_ShaderMatrixSRTTransposeNotsupport));
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSRTTransposeNotsupport *extension = (Extension_ShaderMatrixSRTTransposeNotsupport*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixSRTTransposeNotsupport_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixSRTTransposeNotsupport_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSRTTransposeNotsupport *extension = (Extension_ShaderMatrixSRTTransposeNotsupport*) app->extension;
    ...
    // 四角形描画
    {
        ...
        // 行列をアップロードする
        {
            ...
            // 移動行列
            const mat4 translate = mat4_transpose(mat4_translate(0.5f, 0.5f, 0.0f));
            // 拡大縮小行列
            const mat4 scale = mat4_transpose(mat4_scale(0.5f, 2.0f, 1.0f));
            // 回転行列
            const mat4 rotate = mat4_transpose(mat4_rotate(vec3_create(0.0f, 0.0f, 1.0f), extension->rotate));
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixSRTTransposeNotsupport_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSRTTransposeNotsupport *extension = (Extension_ShaderMatrixSRTTransposeNotsupport*) app->extension;
