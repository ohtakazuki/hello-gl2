 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixRotate_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixRotate_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixRotate *extension = (Extension_ShaderMatrixRotate*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixRotate_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixRotate *extension = (Extension_ShaderMatrixRotate*) app->extension;
