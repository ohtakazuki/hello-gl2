...
typedef struct {
    ...
    // 描画対象のテクスチャ
    Texture *texture;
} Extension_ShaderMatrixScale;

/**
 * アプリの初期化を行う
 */
void sample_ShaderMatrixScale_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_ShaderMatrixScale*) malloc(sizeof(Extension_ShaderMatrixScale));
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixScale *extension = (Extension_ShaderMatrixScale*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixScale_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixScale_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixScale *extension = (Extension_ShaderMatrixScale*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixScale_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixScale *extension = (Extension_ShaderMatrixScale*) app->extension;
