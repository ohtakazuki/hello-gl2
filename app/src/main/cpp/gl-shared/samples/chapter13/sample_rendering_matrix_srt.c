...
typedef struct {
    ...
    // 描画対象のテクスチャ
    Texture *texture;
} Extension_ShaderMatrixSRT;

/**
 * アプリの初期化を行う
 */
void sample_ShaderMatrixSRT_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_ShaderMatrixSRT*) malloc(sizeof(Extension_ShaderMatrixSRT));
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSRT *extension = (Extension_ShaderMatrixSRT*) app->extension;
    ...
    // 頂点シェーダーを用意する
    {
        ...
        //
                "uniform mediump mat4 unif_translate;"
                        ...
                        "void main() {"
                        "   gl_Position = unif_scale * unif_rotate * unif_translate * attr_pos;"
                        "   vary_uv = attr_uv;"
                        "}";
 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixSRT_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixSRT_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSRT *extension = (Extension_ShaderMatrixSRT*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixSRT_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixSRT *extension = (Extension_ShaderMatrixSRT*) app->extension;
