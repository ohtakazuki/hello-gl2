...
typedef struct {
    ...
    // 描画対象のテクスチャ
    Texture *texture;
} Extension_ShaderMatrixTranslate;

/**
 * アプリの初期化を行う
 */
void sample_ShaderMatrixTranslate_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_ShaderMatrixTranslate*) malloc(sizeof(Extension_ShaderMatrixTranslate));
    // サンプルアプリ用データを取り出す
 ...
 * レンダリングエリアが変更された
 */
void sample_ShaderMatrixTranslate_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_ShaderMatrixTranslate_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixTranslate *extension = (Extension_ShaderMatrixTranslate*) app->extension;
...
#else
            mat4 matrix = mat4_translate(0.1f, 0.5f, 0.0f);
#endif
 ...
 * アプリのデータ削除を行う
 */
void sample_ShaderMatrixTranslate_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_ShaderMatrixTranslate *extension = (Extension_ShaderMatrixTranslate*) app->extension;
