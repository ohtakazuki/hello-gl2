...
typedef struct {
    ...
    // 読み込んだテクスチャ
    GLuint texture_id;
} Extension_LoadCompressTexturePvrPvrtc;

/**
 * アプリの初期化を行う
 */
void sample_LoadCompressTexturePvrPvrtc_initialize(GLApplication *app) {
    // 拡張機能チェック
    if (!ES20_hasExtension("GL_IMG_texture_compression_pvrtc")) {
        GLApplication_abortWithMessage(app, "非対応の拡張機能です。\nGL_IMG_texture_compression_pvrtc");
        return;
    }

    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_LoadCompressTexturePvrPvrtc*) malloc(sizeof(Extension_LoadCompressTexturePvrPvrtc));
    // サンプルアプリ用データを取り出す
    Extension_LoadCompressTexturePvrPvrtc *extension = (Extension_LoadCompressTexturePvrPvrtc*) app->extension;
...

    {
        ...
        {
            // PVRTCファイルを読み込む
            PvrtcImage *pvrtc = PvrtcImage_load(app, "texture_pvr_512x512.pvrtc");
            // 正常に読み込まれたかをチェック
            assert(pvrtc != NULL);
            ...
            for (miplevel = 0; miplevel < pvrtc->mipmaps; ++miplevel) {
                ...
                int image_size = pvrtc->image_length_table[miplevel];
                void* image_ptr = pvrtc->image_table[miplevel];

                if (pvrtc->bits_per_pixel == 2) {
                    glCompressedTexImage2D(GL_TEXTURE_2D, miplevel, GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG, width, height, 0, image_size, image_ptr);
                } else {
                    glCompressedTexImage2D(GL_TEXTURE_2D, miplevel, GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG, width, height, 0, image_size, image_ptr);
                }
 ...
 * レンダリングエリアが変更された
 */
void sample_LoadCompressTexturePvrPvrtc_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_LoadCompressTexturePvrPvrtc_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_LoadCompressTexturePvrPvrtc *extension = (Extension_LoadCompressTexturePvrPvrtc*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_LoadCompressTexturePvrPvrtc_destroy(GLApplication *app) {
